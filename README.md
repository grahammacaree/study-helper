...
Write a summary of the lecture in the compose field. Drag the bar on its top edge to grow it for a long write-up. **Symbols** inserts TeX at the caret; the grid is the full set, ordered by this lecture/course/concept and by what you picked this session (not by a model call, and not from your notes). The model checks conceptual mistakes (reduction direction, proven vs conjectured, hardness vs completeness, …) and may note a structural gap. A sign flip or other arithmetic slip in the typed summary is flagged, not stored as shaky. Ordinary spelling typos are ignored unless they are consistent and egregious. Saying a conceptual correction back is optional; **Leave shaky** stores that wobble. **Finish** anytime before a closing quiz. **Side quest** starts a detour from this debrief (or say you want one in the summary).
